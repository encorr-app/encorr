...
def generate_icons():
    ...
    source = PROJECT_ROOT / "assets/encorr_logo.png"

    for size in ICON_SIZES:
        dest_dir = SCRIPT_DIR / f"icons/{size}x{size}"
        dest_dir.mkdir(parents=True, exist_ok=True)
        dest = dest_dir / "encorr.png"
...
def get_file_mappings() -> list[str]:
    ...
    mappings = [
        f"{BUILD_DIR}/=/opt/encorr/",
        f"{SCRIPT_DIR}/app.encorr.encorr.desktop=/usr/share/applications/app.encorr.encorr.desktop",
        f"{SCRIPT_DIR}/encorr.sh=/usr/bin/encorr",
    ]

    for size in ICON_SIZES:
        mappings.append(
            f"{SCRIPT_DIR}/icons/{size}x{size}/encorr.png=/usr/share/icons/hicolor/{size}x{size}/apps/encorr.png"
        )
...
def main():
    ...
    for script in ["encorr.sh", "after-install.sh", "after-remove.sh"]:
